    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            ...
            launch_arguments={
                ...
                'localization':          'true',  # Launch in localization mode
                ...
                'frame_id':              'base_link',  # Fixed frame ID of the robot
                'odom_frame_id':         'odom',  # TF for odometry
                'map_frame_id':          'map',  # Output map frame ID
                'map_topic':             'map',  # Map topic name
                'publish_tf_map':        'true',  # Publish TF between map and odometry
                ...
                'approx_sync':           'false',  # Approximate sync for input topics
                'approx_sync_max_interval': '0.0',  # Max interval for approx sync
                'rgb_topic':             '/zed/microstrain_inertial_driver/rgb/image_rect_color',  # RGB topic
                'depth_topic':           '/zed/microstrain_inertial_driver/depth/depth_registered',  # Depth topic
                'camera_info_topic':     '/zed/microstrain_inertial_driver/depth/camera_info',  # Camera info topic
                ...
                'scan_cloud_topic':      '/scan_cloud',  # Scan cloud topic
                'scan_normal_k':         '0',  # Normal estimation for scan points
                'visual_odometry':       'true',  # Launch visual odometry node
                'icp_odometry':          'false',  # Launch ICP odometry node
                'odom_topic':            '/zed/microstrain_inertial_driver/odom',  # Odometry topic
                #'odom_topic':            '/zed/microstrain_inertial_driver/odom',  # Odometry topic
                'vo_frame_id':           'odom',  # Frame ID for visual/ICP odometry
                'publish_tf_odom':       'true',  # Publish TF for odometry
